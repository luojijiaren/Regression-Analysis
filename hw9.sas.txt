...
data SENIC;
 input ID y x1 x2 x3 x4 x5 x6 x7 x8 x9 x10;
 yprime=log10(y);
cards;
...
proc univariate plots data=SENIC;
  var x1-x10;
run;

proc sgscatter data=SENIC;
  matrix yprime x1-x10;
run;

proc corr data=SENIC cov;
run;

proc reg data=SENIC plot=CriterionPanel;
 model yprime=x1-x10/selection=cp;
run;


proc reg data=SENIC;
 model yprime=x1 x2 x6 x7 x8 x9 x10/ P CLI CLM COLLINOINT STB R VIF;
 model yprime=x1 x2 x4 x6 x7 x8 x9 x10/ P CLI CLM COLLINOINT STB R VIF;
 model yprime=x1 x2 x5 x6 x7 x8 x9/ P CLI CLM COLLINOINT STB R VIF;
run;
quit;
...
proc reg data=validation plot=CriterionPanel;
 model yprime=x1 x4 x8/ P CLI CLM COLLINOINT STB R VIF;
 OUTPUT OUT=d1 PREDICTED=pdc;
run;


data val1;
 set SENIC validation(drop=yprime);
run;

proc reg data=val1 plot=CriterionPanel;
 model yprime=x1 x4 x8/ P CLI CLM COLLINOINT STB R VIF;
 OUTPUT OUT=d1 PREDICTED=pdc;
run;
